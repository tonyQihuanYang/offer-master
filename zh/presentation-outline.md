...
> ⏱️ **时间纪律**：UC1 约 32 分钟，UC2 约 22 分钟，开场+收尾约 6 分钟。
...
| 维度 | A (Template DSL) | B (Raw + Mobile) | **C (Hybrid) ✅** |
|------|------------------|------------------|-------------------|
| 实验速度 | 快 | 慢（要发版） | 快（layout）/ 慢（仅新组件） |
| 200ms 风险 | 高（服务端渲染） | 低 | 低 |
| Mobile 复杂度 | 最低 | 最高 | 中（有界） |
| 原生 UX | 差 | 极好 | 极好 |
| iOS/Android 一致性 | 保证 | 难 | 由共享组件契约保证 |
| 契合现状 | 新范式 | 自然延伸 | 自然延伸 + 轻结构 |

- **A vs Hybrid 的唯一本质区别：presentation logic 在哪？** A=服务端格式化后发字符串；C=发原始数据，mobile 格式化
- 推荐 **C**：拿到 A 的"实验不发版"，又保住 B 的"原生 UX"，且最贴合"系统已经在推 JSON"的现状
...
- **🔑 最关键的一问**："Flink 真的适合我们的事件率吗?测过吗?" → 50K 快递员 × ~30 单/天 ≈ **150 万事件/天 ≈ 平均 20 事件/秒、峰值 ~100/秒**;**Flink 是为 100k+ 事件/秒设计的——团队在为用不到的容量交复杂度税** ⚠️ **注:UC2 没给体量,这个数是借 UC1 的 5 万快递员推算的——所以正确说法是"先测真实事件率",别把估算当题目条件。**
...
- **与 Leadership(PM 正在升级到的人)**:**抢在 PM 之前,和 TM 一起 brief**:
  > "我们发现团队为一个比 Flink 设计点小 50-100 倍的负载选了 Flink,正在交复杂度税、拖慢交付。我们已把 sprint 砍到 1 个模式,用接下来 30 天正式评估架构是否要换,[日期]前给建议。希望你在这期间给我们对 PM 的 air cover。"
...
| Flink 到底对不对? | 团队事件率比 Flink 设计点小 50-100 倍——但我的工作不是宣判它错,是帮团队自己测出来、自己得出结论。 |
