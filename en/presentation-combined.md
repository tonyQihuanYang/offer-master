...
**🖥️ On screen:** 15 countries · 50k couriers · 2M/h (~556 RPS) · 200ms p95 (today ~180 → ~20ms headroom) · today = one hardcoded template, no A/B testing.
...
**🖥️ On screen:** buckets: architecture (45s — measured?), the big one (Flink right for our rate? no number given → measure), **scope — audit the 12 first** (dupes / subsets / data-gaps), data quality, testing.
...
> On scope, I don't just ask *'which 3 of 12.'* I **audit the 12 first** with the team. Often it's really 4–5 distinct patterns — some are duplicates, subsets, or need data we don't have. That's engineering the scope **down**, not just picking from a list.
...
**🖥️ On screen:** ship 1 pattern (">500m from destination"). Day 1 TM 1:1 + audit Flink telemetry + **audit the 12 patterns** + scope-lock in writing. Day 2 skeleton + fixture. Day 3 dry run + pre-brief director. Team presents.
...
> ② **The 12 patterns themselves** — duplicates? subsets of each other? need data we don't have? Often '12' collapses to 4–5 real ones. That's engineering the scope down, not just picking from a list.
